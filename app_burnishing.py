...
MAINFOLDER = r"D:\APPdata_burnishing"
...
##    st.error("No data found or processed. Please check files and asset names.")
        ...
        combined_columns = ['Date', 'Time', 'Sr No'] + ['tot_acc', 'ver_vel', 'ax_vel', 'hor_vel', 'temp'] * len(asset_order)
        ...
        for asset in asset_order:
            ...
            if not asset_found:
                missing_data = pd.DataFrame(0, index=range(len(combined_df)), columns=['tot_acc', 'ver_vel', 'ax_vel', 'hor_vel', 'temp'])
...
def train_ensemble_model(training_file_path, model_folder_path):
    ...
    def preprocess_data(X, y):
        ...
        # Save the scaler
        joblib.dump(scaler, os.path.join(model_folder_path, "scaler_burnishing.pkl"))
    ...
    class_weights_nn = {0: 1.0, 1: 30}

    # Build Keras model
    nn_model = KerasClassifier(model=build_nn_model, epochs=100, batch_size=32, verbose=0, class_weight=class_weights_nn)
    ...
    joblib.dump(ensemble_model, os.path.join(model_folder_path, "ensemble_burnishing.pkl"))
...
def predict_ensemble(test_file_path, model_folder_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, "scaler_burnishing.pkl"))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict(X_test_scaled):
        ensemble_model = joblib.load(os.path.join(model_folder_path, "ensemble_burnishing.pkl"))
...
def train_model(training_file_path):
    def load_data(file_path):
        ...
        #y = df.iloc[:, 37].values
        y = df['TimeToBreakdown_Hours'].values  # Target column
        return X, y
